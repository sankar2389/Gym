...
<a class="left carousel-control" href="#myCarousel" role="button" data-slide="prev">
 ...
 <div class="gym_cat_select">  
            ...
            <span id='toggleBtn' style="width:50%; float:left; padding:15px 0px;text-align:right;">
                ...
                <input id='toggleChk'   name='toggleChk'  data-toggle="toggle" data-on="Personal trainer" data-off="Self-train"  data-onstyle="success" data-offstyle="danger" type="checkbox">
            </span>
                     ...
                     if($PriceListvalue->pack_desc !="" && $PriceListvalue->price >0)
                      {
                            ...
                            if($PriceListvalue->tra_id==1) //with trainer
                            {
                               ...
                               $trainOnCnt++;
                               $priceDesc='With Personal training';

                            }
                            elseif($PriceListvalue->tra_id==2)
                            {
                              $trainerClass='trainerOFF';
                               $priceDesc='Self-training';
                            }
    ...
    interval: false
        ...
        $("#selPak").change(function() {
            ...
            if(PakId>0 && rWID>0 && selCatid>0){
              $('#toggleBtn').html("<input id='toggleChk'   name='toggleChk'  data-toggle='toggle' data-on='Personal trainer' data-off='Self-train'  data-onstyle='success' data-offstyle='danger' type='checkbox'>");
                $(".se-pre-con").show()
                $('#PakTitle').html("");
                $.ajax({  
                    type: "POST",
                         ...
                         for(var i=0; i<data.resultSet.length; i++)
                         {
                            if(data.resultSet[i].price>0){
                             ...
                             if(data.resultSet[i].tra_id==1)
                             {
                                ...
                                priceDescVal='With Personal training';
                                withTrainCnt++;
                             }
                             else if(data.resultSet[i].tra_id==2) 
                             {
                                trainerCls='trainerOFF';
                                priceDescVal='Self-training';
                             }
                             ...
                             pushPrice+="<div class='row "+trainerCls+"'><div class='package_div'><div class='package_name'> "+priceDescVal+" </div><div class='rate_box'> Rs."+data.resultSet[i].price+"/per "+perDis+" </div><button class='btn btn-info btn-sm payBtn' lang='"+pricerwId+"'>Buy now</button></div></div>";
                         ...
                         for(var i=0;i<data.getListMinPackage.length;i++ )
                         {
                             ...
                             else if(data.getListMinPackage[i].tra_id==2) 
                             {
                               ...
                               var pricerwId=data.getListMinPackage[i].pak_price_id;
                                 pushPrice+="<div class='row "+trainerCls+"'><div class='package_div'><div class='package_name'> "+priceDescVal+" </div><div class='rate_box'> Rs."+data.getListMinPackage[i].price+"/per "+perDis+" </div><button class='btn btn-info btn-sm payBtn' lang='"+pricerwId+"'>Buy now</button></div></div>";
                             }
                         }
