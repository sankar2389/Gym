                     ...
                     foreach ($KeyVals->Banner as $key => $Bannervalue) {
                         ...
                         if($rKryVal==1){
                            ...
                            <span class="tab-pane fade in active" id="tab<?php echo $rKryVal; ?>">
                                <img src="<?php   echo url("uploads/images/".$Bannervalue->img_path); ?>" class="img-responsive">
                            </span>
                            <?php
                         }
                         else
                                    ...
                                    ?>
                                     <div class="col-sm-3 col-xs-3">
                                        <a  href="#tab<?php echo $tabId; ?>" data-toggle="tab">
                                            <img class="img-responsive" width='100px' src="<?php   echo url("uploads/thumbnail/".$value->img_path); ?>">
                                        </a>
                ...
                <div class="col-md-7 col-sm-7 col-xs-12">
                    ...
                    <div class="header_panel">
                        ...
                        <div class="gym_cat_select">  
                            <form name='list_gymPay' id='list_gymPay'  method='post' class="form-inline">
                ...
                $.ajax({  
                    ...
                    { $(".se-pre-con").fadeOut("slow"); 
                     ...
                     {  
                         ...
                         {
                             DivTitle=data.resultSet[i].package_name;
                             pushPrice+="<div class='row'><div class='package_div'><div class='package_name'> "+data.resultSet[i].pack_desc+" </div><div class='rate' style='width: 30% !important'> Rs."+data.resultSet[i].price+"/Hr </div></div>&nbsp;<button class='btn btn-info btn-sm'>Pay Now</button></div>";
            ...
            if(selCatid!=Cat)
            {
                ...
                $.ajax({  
                    ...
                    { $(".se-pre-con").fadeOut("slow"); 
                     if(data.error==1)
                     {
                         ...
                         for(var i=0;i<data.getListMinPackage.length;i++ )
                         {
                             if(data.getListMinPackage[i].price>0){
                                 pushPrice+="<div class='row'><div class='package_div'><div class='package_name'> "+data.getListMinPackage[i].pack_desc+" </div><div class='rate' style='width: 30% !important'> Rs."+data.getListMinPackage[i].price+"/Hr </div></div>&nbsp;<button class='btn btn-info btn-sm'>Pay Now</button></div>";
                             }
                         }
                     ...
                     /* course_options.empty();
				course_options.append($("<option />").val("0").text("--Select--"));
				$.each(data, function() {
				course_options.append($("<option />").val(this.course_id+'###'+this.subject_id).text(this.course_code));
...
$(".se-pre-con").show()
    $.ajax({  
            type: "POST",
            url: "{{url('/gymCatPricessssssssssssss')}}", 
