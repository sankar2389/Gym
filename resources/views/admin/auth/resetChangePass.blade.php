...
<div class="row">
        <div class="col-sm-6 col-md-4 col-md-offset-4">
            <div class="account-wall" style='text-align: center;'>
                ...
                <input type="hidden" id='hidKey' name='hidKey' value={{$keyId}}>
                <input type="password" class="form-control" id='npass' name='npass' placeholder="Enter new password" required>
                 <br/>
                <input type="password" class="form-control" id='renpass' name='renpass' placeholder="Confirm password" required>
                <br/>
...
$('#errMsg').html("");
    ...
    rules: { 
                        npass:{required: true,minlength: 8},
                         renpass:{required: true,equalTo: "#npass"},
    ...
    messages: {
               npass:{ required : "Enter admin password",minlength: "Minimum length eight and above"     }, 
renpass:{required:"Enter Confirm Password Same as Password"}
...
$('#errMsg').html("");
          ...
          if(data==0)
            {
             $('#errMsg').html("<font color='red'>Invalid Request</font>");;
            }
            else if(data==1)
            {
              $('#errMsg').html("<font color='red'>Invalid request or already key used</font>"); 
            }
            else if(data==3)
            {
              $('#errMsg').html("<font color='green'>Your password changed successfully</font>"); 
            }
            else
            {
               $('#errMsg').html("<font color='red'>Invalid Request</font>"); 
